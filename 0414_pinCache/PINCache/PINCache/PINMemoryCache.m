...
- (void)removeObjectAndExecuteBlocksForKey:(NSString *)key
{
    ...
    [self lock]; // 加锁
        if (cost)
            _totalCost -= [cost unsignedIntegerValue]; // 内存缓存容量更新
        // 删除缓存容量对应的数据
...
- (__nullable id)objectForKey:(NSString *)key
{
    ...
    [self lock]; // 加锁
        ...
        if (!self->_ttlCache || self->_ageLimit <= 0 || fabs([[_dates objectForKey:key] timeIntervalSinceDate:now]) < self->_ageLimit) {
            object = _dictionary[key]; // 取值
        }
    [self unlock];
        
    if (object) {
        [self lock];
            _dates[key] = now; // 更新缓存的访问时间
        [self unlock];
    }
...
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost
{
    ...
    [self lock];
        ...
        _dictionary[key] = object; // 将数据缓存到字典里
        _dates[key] = [[NSDate alloc] init]; // 设置数据的缓存时间
        _costs[key] = @(cost); // 设置数据容量大小
        
        _totalCost += cost; // 总容量大小
    [self unlock];
    
    if (didAddObjectBlock)
        didAddObjectBlock(self, key, object);
    
    if (costLimit > 0) // 判断内存缓存的容量是否超出限制
