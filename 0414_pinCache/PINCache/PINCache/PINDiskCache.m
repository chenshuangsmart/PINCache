...
- (__nullable id <NSCoding>)objectForKey:(NSString *)key fileURL:(NSURL **)outFileURL
{
    ...
    [self lock]; // 加锁
        fileURL = [self encodedFileURLForKey:key]; // 根据 key算出对应的磁盘缓存路径
        object = nil;
        // 判断文件是否存在
        if ([[NSFileManager defaultManager] fileExistsAtPath:[fileURL path]] &&
            ...
            @try {
                object = [NSKeyedUnarchiver unarchiveObjectWithFile:[fileURL path]]; // 读出文件内容到 object 对象
            }
          ...
          if (!self->_ttlCache) {
            [self setFileModificationDate:now forURL:fileURL]; // 修改该文件的读取时间
          }
        }
    [self unlock]; // 解锁
...
{
    NSDate *now = [[NSDate alloc] init]; //当前时间
    if (!key || !object)    // 值和健都不能为空
        return;
    PINBackgroundTask *task = [PINBackgroundTask start]; // 启动后台任务
    #if TARGET_OS_IPHONE
    ...
    NSURL *fileURL = nil;
    [self lock]; // 加锁
        fileURL = [self encodedFileURLForKey:key];  // 根据key 算出对应的磁盘缓存路径
        if (self->_willAddObjectBlock) // 判断设置设值之前要执行的 block 是否为空
            self->_willAddObjectBlock(self, key, object, fileURL);
  
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:object];
        NSError *writeError = nil;
        BOOL written = [data writeToURL:fileURL options:writeOptions error:&writeError]; // 将数据写入磁盘
        PINDiskCacheError(writeError);
        if (written) {
            [self setFileModificationDate:now forURL:fileURL]; // 设置文件修改日期
            // 算出写入磁盘的缓存容量大小
            NSError *error = nil;
            NSDictionary *values = [fileURL resourceValuesForKeys:@[ NSURLTotalFileAllocatedSizeKey ] error:&error];
            PINDiskCacheError(error);
            
            NSNumber *diskFileSize = [values objectForKey:NSURLTotalFileAllocatedSizeKey]; // 取出文件的大小
            if (diskFileSize) {
                NSNumber *prevDiskFileSize = [self->_sizes objectForKey:key]; // 该 key 对应的缓存容量大小
                if (prevDiskFileSize) { // 如果这个 key之前已经被使用过了,则更新该 key 对应的缓存容量大小
                    self.byteCount = self->_byteCount - [prevDiskFileSize unsignedIntegerValue];
                }
                // 该 key之前没有被使用过,把该 key对应的缓存容量大小记录保存下来
                [self->_sizes setObject:diskFileSize forKey:key];
                self.byteCount = self->_byteCount + [diskFileSize unsignedIntegerValue]; // atomic 总容量大小
            }
            if (self->_byteLimit > 0 && self->_byteCount > self->_byteLimit) // 磁盘缓存容量是否超出限制
                [self trimToSizeByDate:self->_byteLimit block:nil]; // 按缓存最久的数据最先移除的策略来删除缓存数据
        } else {
            fileURL = nil;
        }
        if (self->_didAddObjectBlock) // 判断设值之后的block 是否为空
...
- (void)removeObjectForKey:(NSString *)key fileURL:(NSURL **)outFileURL
{
    ...
    PINBackgroundTask *task = [PINBackgroundTask start]; // 开始执行后台任务
    
    NSURL *fileURL = nil;
    
    [self lock];
        fileURL = [self encodedFileURLForKey:key]; // 根据 key取出缓存路径
        [self removeFileAndExecuteBlocksForKey:key]; // 移除缓存
    [self unlock];
